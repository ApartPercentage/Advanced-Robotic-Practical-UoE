...
def solution():
    ...
    cubePos -= np.array([0, 0, 0.85])
    cubePos -= np.array([0, 0, 0.09])
    targetPos = finalTargetPos -  np.array([0, 0, 0.85])
    #half x-size of box
    targetPos -= np.array([0.035, 0, 0])
    #half x-size of lhand
    targetPos -= np.array([0.0285, 0, 0])
    sim.pushCube(cubePos, targetPos)
